...
public class Waiting extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			} else if (approve.equalsIgnoreCase("Delete")) {
				String query1 = "DELETE FROM Award WHERE suid='" + name.trim()
						+ "' AND ruid='" + details.trim() + "' AND sdept='"
						+ sdept.trim() + "' AND rdept='" + rdept.trim()
						+ "' AND award='" + award.trim() + "' AND smail='"
						+ smail.trim() + "' AND rmail='" + rmail.trim() + "'";
				// Execute the query
				int rs1 = stmt.executeUpdate(query1);
				if (rs1 == 1) {
					out.print("Deleted Specific Award");
					RequestDispatcher rd = request
							.getRequestDispatcher("./entranceAdminFunction.jsp");
					rd.include(request, response);
				} else {
					out.print("Cannot Delete Award");
					RequestDispatcher rd = request
							.getRequestDispatcher("./entranceAdminFunction.jsp");
					rd.include(request, response);
				}
